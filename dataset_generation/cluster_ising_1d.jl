...
for qubits in qubits_num
    ...
    for i=1:length(h1)
        for j=1:length(h2)
            ...
            push!(corrzz_list, compute_correlation_pauliz_norm(psi))
    ...
    write_dataset_to_csv_cluster_ising(dataset, "dataset_generation/dataset_results/ising_1d/Ci_n($qubits)_X(h1, h2)_y(energy,entropy,corrzz).csv")
end
